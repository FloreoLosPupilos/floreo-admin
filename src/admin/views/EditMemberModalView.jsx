...
import { fileUpload, setMembersCustomId } from "../../helpers";
...
export const EditMemberModalView = (props) => {
  ...
  const onSubmit = async ({ name, contact, phone }) => {
    if (memberImage != props.data.img) {
      ...
      setMembersCustomId('Integrantes', { nombre: name, contacto: contact, telefono: phone, img: url }, props.data.id, props.dis);

      window.dispatchEvent(evt);
      Swal.fire('Integrante Modificado', 'Se actualizó correctamente el integrante', 'success');
    } else {
      setMembersCustomId('Integrantes', { nombre: name, contacto: contact, telefono: phone, img: props.data.img }, props.data.id, props.dis)
